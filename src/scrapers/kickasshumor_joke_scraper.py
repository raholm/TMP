...
class KickassHumorJokeScraper(object):
    ...
    def _write_jokes_to_file(self):
        file = os.path.join(get_project_data_path(), "kickasshumor_raw", "%s.json" % self.category.replace("-", "_"))
...
def main():
    ...
    scraped_categories = {category.split(".")[0]
                          for category in os.listdir(os.path.join(get_project_data_path(), "kickasshumor_raw"))}
